...
class ContactController extends \yupe\components\controllers\FrontController
{
            ...
            if ($form->validate()) {
                ...
                if ($this->feedback->send($form, $module)) {
                    ...
                    if (Yii::app()->getRequest()->getIsAjaxRequest()) {
                        Yii::app()->ajax->success(Yii::t('OtherModule.other', 'Your message sent! Thanks!'));
                    }

                    Yii::app()->getUser()->setFlash(
                        YFlashMessages::SUCCESS_MESSAGE,
                        Yii::t('OtherModule.other', 'Your message sent! Thanks!')
                    );
                    ...
                    if (Yii::app()->getRequest()->getIsAjaxRequest()) {
                        Yii::app()->ajax->failure(
                            Yii::t('OtherModule.other', 'It is not possible to send message!')
                        );
                    }
    ...
    public function actionFaqView($id)
    {
        ...
        if (empty($id)) {
            throw new CHttpException(404, Yii::t('OtherModule.other', 'Page was not found!'));
        }
