...
class ContentBlockBackendController extends yupe\components\controllers\BackController
{
    ...
    public function actionCreate()
    {
        ...
        if (!empty($id) && !empty($lang)) {
            $contentBlock = ContentBlock::model()->findByPk($id);

            if (null === $contentBlock) {
                Yii::app()->getUser()->setFlash(
                    yupe\widgets\YFlashMessages::ERROR_MESSAGE,
                    Yii::t('ContentBlockModule.contentblock', 'Targeting block was not found!')
                );

                $this->redirect(['/contentblock/contentBlockBackend/create']);
            }

            if (!array_key_exists($lang, $languages)) {
                Yii::app()->getUser()->setFlash(
                    yupe\widgets\YFlashMessages::ERROR_MESSAGE,
                    Yii::t('ContentBlockModule.contentblock', 'Language was not found!')
                );
