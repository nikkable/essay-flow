...
class OrderBackendController extends yupe\components\controllers\BackController
{
    ...
    public function actionAuthors()
    {
        ...
        $this->render('authors', ['model' => $model]);
    }

    /**
     *
     */
    public function actionAuthorsPayments()
    {
        ...
        $this->render('authorsPayments', ['model' => $model]);
    }
     ...
     * @throws CHttpException
     */
    public function actionAuthorsPaymentsDelete($id)
    {
        $model = $this->loadModelAuthorPayments($id);
        ...
        if (Yii::app()->getRequest()->getIsPostRequest()) {
            ...
            if (!Yii::app()->getRequest()->getQuery('ajax')) {
                $this->redirect(Yii::app()->getRequest()->getPost('returnUrl', ['authorsPayments']));
            }
        } else {
            throw new CHttpException(400, Yii::t('OrderModule.order', 'Unknown request. Don\'t repeat it please!'));
        }
     ...
     * @throws CHttpException
     */
    public function actionAuthorsPaymentsDone($id)
    {
        $model = $this->loadModelAuthorPayments($id);
        ...
        if(!($model->hold > 0)) {
            Yii::app()->getUser()->setFlash(
                YFlashMessages::ERROR_MESSAGE,
                Yii::t('OrderModule.order', 'No hold balance')
            );

            $this->redirect(Yii::app()->getRequest()->getPost('returnUrl', ['authorsPayments']));
        }

        $model->paid = $model->hold;
        $model->hold = 0;

        if($model->save(false)) {
            Yii::app()->getUser()->setFlash(
                YFlashMessages::SUCCESS_MESSAGE,
                Yii::t('OrderModule.order', 'Pay out!')
            );
        }
     ...
     * @throws CHttpException
     */
    public function loadModelAuthorPayments($id)
    {
        $model = AuthorPayments::model()->findByPk($id);
