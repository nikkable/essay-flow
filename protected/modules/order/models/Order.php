...
class Order extends yupe\models\YModel
{
     ...
     * @var null
     */
    public $rackcalcPeriod = null;
    public $rackcalcSubject = null;
    public $rackcalcWord = null;
    public $rackcalcLanguage = null;
...
//            ['status_id, delivery_id', 'required'],
            ...
            ['email', 'email'],
            [
                'delivery_id, separate_delivery, payment_method_id, paid, user_id, couponId, manager_id, author_id',
                'numerical',
                'integerOnly' => true,
            ],
            ...
            [
                'id, delivery_id, delivery_price, payment_method_id, paid, payment_time, payment_details, total_price, discount, coupon_discount, separate_delivery, status_id, date, user_id, name, phone, email, comment, ip, url, note, modified, manager_id, tax, author_id',
                'safe',
                'on' => 'search',
            ],
            ['file, tax, katarun_id', 'safe'],
            ['terms', 'compare', 'compareValue' => true, 'message' => Yii::t('OtherModule.other', 'You must agree to the terms and conditions')],
        ];
    }
    ...
    public function attributeLabels()
    {
        return [
            ...
            'house' => Yii::t('OrderModule.order', 'State'),
            'apartment' => Yii::t('OrderModule.order', 'Apartment'),
            'manager_id' => Yii::t('OrderModule.order', 'Manager'),
            'file' => Yii::t('OrderModule.order', 'File'),
            'tax' => Yii::t('OrderModule.order', 'Tax'),
            'coff' => Yii::t('OrderModule.order', 'Coefficient'),
            'currency' => Yii::t('OrderModule.order', 'Currency'),
            'author_id' => Yii::t('OrderModule.order', 'Author Id'),
        ];
    }
    ...
    {
        return [
            self::PAID_STATUS_PAID => Yii::t("OtherModule.other", 'Paid'),
            self::PAID_STATUS_NOT_PAID => Yii::t("OtherModule.other", 'Not paid'),
        ];
    }
    ...
    {
        return (float)$this->discount + (float)$this->coupon_discount;
    }
    ...
    {
        return round(((float)$this->discount + (float)$this->coupon_discount) * Yii::app()->controller->currencyCoff, 2);;
    }
    ...
    {
        $total = (float)$this->total_price - (float)$this->discount - (float)$this->coupon_discount;
    ...
    {
        return round((float)$this->total_price * Yii::app()->controller->currencyCoff, 2);
    }
    ...
    {
        return round($this->getTotalPriceWithDelivery() * Yii::app()->controller->currencyCoff, 2);
    }

    /**
     * @return string
     */
    public function getTotalPriceCurrencyFormat()
    {
        return $this->getTotalPriceCurrency() . ' ' . Yii::app()->controller->currency;
    }
    ...
    {
        return round($this->getTotalPriceCurrency() / 100 * Yii::app()->getModule('order')->tax, 2);
    }

    /**
     * @return string
     */
    public function getTotalTaxFormat()
    {
        return Yii::app()->getModule('order')->tax > 0 ? $this->getTotalTax() . ' ' . Yii::app()->controller->currency : 'n/a';
    }
    ...
    public function pay(Payment $payment, $paid = self::PAID_STATUS_PAID)
    {
        ...
        $this->setAttributes([
            ...
            'tax' => $this->total_price / 100 * Yii::app()->getModule('order')->tax,
        ]);
    ...
    public function getFilePath()
    {
        ...
        return '/'.Yii::app()->getModule('yupe')->uploadPath.'/'.Yii::app()->getModule('order')->uploadPath.'/'.$this->file;
    }

    public function getOrderNumber()
    {
        return Yii::app()->params['orderSymbol'] . '-' . (Yii::app()->params['orderNumber'] * $this->id);
    }
    ...
    public static function authorList()
    {
        ...
        $sql = "
            ...
            WHERE ur.itemname = :roleName
        ";

        $users = Yii::app()->db->createCommand($sql)->queryAll(true, array(':roleName' => $roleName));

        return CHtml::listData($users, 'id', 'nick_name');
    }
}
