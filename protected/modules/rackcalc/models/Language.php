<?php
/**
 * This is the model class for table "{{rc_language}}".
 *
 * The followings are the available columns in table '{{rc_language}}':
 * @property integer $id
 ...
 *
 */
class Language extends yupe\models\YModel
{
    /**
...
//            ['name, code', 'filter', 'filter' => [$obj = new YPurifier(), 'purify']],
            ['code', 'length', 'max' => 100],
    ...
    public function languageCriteria($lang)
    {
    ...
    {
        $lists = self::model()->languageCriteria(Yii::app()->getLanguage())->findAll();

        if(count($lists) === 0) {
            return self::model()->languageCriteria(Yii::app()->getModule('yupe')->defaultLanguage)->findAll();
        }

        return $lists;
    }

    public static function getLanguagesDropdown()
    {
        $lists = self::model()->languageCriteria(Yii::app()->getLanguage())->findAll();
