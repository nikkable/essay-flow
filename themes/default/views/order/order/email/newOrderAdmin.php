...
<?php $subject = Yii::t(
    'OtherModule.other',
    Yii::t("OtherModule.other", "Order") . ' #{n} on "{site}"',
    ['{n}' => $order->id, '{site}' => Yii::app()->getModule('yupe')->siteName]
); ?>
<h1 style="font-weight:normal;">
    <?= CHtml::link(
        Yii::t("OtherModule.other", "Order") . " №{$order->id}",
        Yii::app()->createAbsoluteUrl('/order/orderBackend/update', ['id' => $order->id])
    ); ?>
    <?= Yii::t("OtherModule.other", "for the amount"); ?> <?= $order->getTotalPriceCurrencyFormat(); ?>
</h1>
<table cellpadding="6" cellspacing="0" style="border-collapse: collapse;">
    ...
    <tr>
        <td style="padding:6px; width:170px; background-color:#f0f0f0; border:1px solid #e0e0e0;">
            <?= Yii::t("OtherModule.other", "Status"); ?>
        </td>
    ...
    <?php if ($order->comment): ?>
        <tr>
            <td style="padding:6px; background-color:#f0f0f0; border:1px solid #e0e0e0;">
                <?= Yii::t("OtherModule.other", "Comment"); ?>
            </td>
            <td style="padding:6px; background-color:#ffffff; border:1px solid #e0e0e0;">
...
<table cellpadding="6" cellspacing="0" style="border-collapse: collapse;">
    ...
    <?php if ($order->hasCoupons()): ?>
        <tr>
            ...
            <td align=right style="padding:6px; text-align:right; width:170px; background-color:#ffffff; border:1px solid #e0e0e0;">
